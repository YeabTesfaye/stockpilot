...
import { scheduleProduction } from '@/server/planning/schedule';
...
export async function GET() {
  ...
  return withTenant(tenantId, async () => {
    try {
      const grid = await scheduleProduction(tenantId);
      return NextResponse.json({ grid, generatedAt: new Date().toISOString() });
    } catch (err) {
