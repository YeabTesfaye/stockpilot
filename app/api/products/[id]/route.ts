...
export async function GET(
  ...
  return withTenant(tenantId, async () => {
    const item = await products.getProduct(id);
...
export async function PUT(
  ...
  if (!can(session.roleBindings, Action.UPDATE_PRODUCT)) {
    return NextResponse.json({ error: 'Forbidden' }, { status: 403 });
  }
  ...
  return withTenant(tenantId, async () => {
    try {
      const item = await products.updateProduct(id, {
...
export async function DELETE(
  ...
  if (!can(session.roleBindings, Action.DELETE_PRODUCT)) {
    return NextResponse.json({ error: 'Forbidden' }, { status: 403 });
  }
